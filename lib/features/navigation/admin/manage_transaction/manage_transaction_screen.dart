...
class ManageTransactionScreen extends StatefulWidget {
  ...
  const ManageTransactionScreen({super.key,this.statusFilter});
...
class _ManageTransactionScreenState extends State<ManageTransactionScreen> {
  ...
  late AllTransactionsProvider _provider;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _provider = context.read<AllTransactionsProvider>();

      // // initial load
      // filtered = List.from(_provider.allTransactions);

      // listen for future updates
      _provider.addListener(_onTransactionsUpdated);

      if (_provider.allTransactions.isNotEmpty || !_initialized) {
        setState(() {
          filtered = List.from(_provider.allTransactions);
          _initialized = true;
        });
  ...
  void _onTransactionsUpdated() {
    if (!_initialized && _provider.allTransactions.isNotEmpty) {
      setState(() {
        filtered = List.from(_provider.allTransactions);
        _initialized = true;
      });
  ...
  @override
  void dispose() {
    _provider.removeListener(_onTransactionsUpdated);
    super.dispose();
  }
